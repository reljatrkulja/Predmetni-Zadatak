...
public class dialogSoftware extends JDialog implements ActionListener {
	...
	public dialogSoftware(Frame1 parent, String title, boolean modal, boolean edit) {
		...
		JCheckBox max = new JCheckBox();
		max.setText("Extrude");
		//alatiDugmad.add(max);
		polja.add(max);

		JCheckBox blender = new JCheckBox();
		blender.setText("Inset");
		//alatiDugmad.add(blender);
		polja.add(blender);

		JCheckBox zbrush = new JCheckBox();
		zbrush.setText("Paint");
		...
		btnOk.addActionListener(x -> {
			Softver softverish = null;
			if (edit == true) {
				int i = parent.getSelectedIndex();
				softverish = Utils.getSoftveri().get(i);
			}
			
			System.out.println("ovo radi");
			System.out.println(nazivPolje.getText());

			if (nazivPolje.getText().equals("") || fajlFormatPolje.getText().equals("")
					|| nazivCetkice.getText().equals("")) {
				...
				String izabranAlat = new String("");
				
				if(zbrush.isSelected()) {
					String brush = new String("Paint, ");
					izabranAlat = izabranAlat + brush;
				}
				if(blender.isSelected()) {
					String brush = new String("Inset, ");
					izabranAlat = izabranAlat + brush;
				}
				if(max.isSelected()) {
					String maxc = new String("Extrude, ");
					izabranAlat = izabranAlat + maxc;
				}
				final String finala = izabranAlat;
				
				String softverAlati = finala;
				System.out.println("alati su: " + finala);

				Boolean istoIme = false;

				int velicinaSoftver = Utils.getSoftveri().size();
				for (int i = 0; i < velicinaSoftver; i++) {
					if (softverNaziv.equals(Utils.getSoftveri().get(i).getNaziv())) {
						istoIme = true;
					}
				}

				if (istoIme == true && !softverish.getNaziv().equals(softverNaziv)) {
					JOptionPane.showMessageDialog(parent, "Error: Unet je vec postojeci softver");
				} else {
