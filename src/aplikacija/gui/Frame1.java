...
import aplikacija.dialog.dialogCetkica;
import aplikacija.dialog.dialogRender;
...
//		Pravljenje tabele softvera		
		...
		softwareTable.addMouseListener(new MouseAdapter() {
			public void mouseClicked(java.awt.event.MouseEvent arg0) {
			    int row = softwareTable.rowAtPoint(arg0.getPoint());
			    int col = softwareTable.columnAtPoint(arg0.getPoint());
				if (arg0.getClickCount() == 2) {

				    System.out.println("kliknut je " + row + " red i " + col + " kolona");
				    if (col == 1) {
				    	dialogCetkica dialogcetkica = new dialogCetkica(getSelectedIndex());
				    	dialogcetkica.setVisible(true);
				    }
				    if(col == 4) {
				    	dialogRender render = new dialogRender(getSelectedIndex());
